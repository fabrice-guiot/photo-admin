...
class S3Adapter(StorageAdapter):
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                logger.info(
                    ...
                    extra={"bucket": bucket, "prefix": prefix, "file_count": len(files)}
                )
                return files
                ...
                if error_code in ["AccessDenied", "InvalidAccessKeyId", "SignatureDoesNotMatch"]:
                    logger.error(f"S3 permission error: {error_code}", extra={"bucket": bucket})
                ...
                if attempt < self.MAX_RETRIES - 1:
                    ...
                    logger.warning(
                        f"S3 list_files attempt {attempt + 1} failed, retrying in {backoff}s",
                ...
                logger.error(f"S3 connection error: {str(e)}", extra={"bucket": bucket})
    ...
    def test_connection(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.info("S3 connection test successful", extra={"bucket_count": bucket_count})
            ...
            logger.error(f"S3 connection test failed: {error_code}", extra={"error": error_msg})
            ...
            logger.error(f"S3 connection test failed: Cannot reach endpoint - {str(e)}")
            return False, f"Cannot connect to S3 endpoint. Check network connectivity: {str(e)}"

        except Exception as e:
            logger.error(f"S3 connection test failed with unexpected error: {str(e)}")
