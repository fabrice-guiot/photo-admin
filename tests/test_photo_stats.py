...
class TestHelpText:
    ...
    def test_h_flag_works_identically(self):
        ...
        h_result = subprocess.run(
            [sys.executable, 'photo_stats.py', '-h'],
            capture_output=True,
            text=True
        )
    ...
    def test_help_contains_required_elements(self):
        ...
        assert 'Analyze photo collections' in help_text
        assert 'orphaned files' in help_text
        ...
        assert 'photo_stats_report' in help_text
